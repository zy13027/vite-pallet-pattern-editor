import "./style.css";
import { ApiLogin, PlcProgramWrite, PlcProgramRead, RequestConfig } from '@siemens/simatic-s7-webserver-api';
...
type Box = {
    ...
    y: number; // center (mm)
    w: number; // mm
    d: number; // mm (Mapped to 'l' in PLC)
    rot: Rot;
};
...
class PlcManager {
    ...
    // Updated DB Name based on your file
    ...
    async login(user: string, pass: string): Promise<boolean> {
        try {
            ...
            if (response.result) {
                ...
                console.log("PLC Token received:", this.authToken);
                return true;
            }
    ...
    async writePattern(boxes: Box[]) {
        ...
        // Constraint: UDT_typeWebProduct array is defined as [1..20]
        if (boxes.length > 20) {
            alert(`PLC Limit: The DB only supports 20 boxes. You have ${boxes.length}.`);
            return false;
        }
        ...
        // 1. Set useWebPattern to TRUE
        promises.push(writer.write(`${this.dbName}.useWebPattern`, true));

        // 2. Write web_ProductCount
        promises.push(writer.write(`${this.dbName}.web_ProductCount`, boxes.length));

        // 3. Write web_data Array
        boxes.forEach((b, i) => {
            const idx = i + 1; // PLC arrays are 1-based
            const prefix = `${this.dbName}.web_data[${idx}]`;

            promises.push(writer.write(`${prefix}.x`, b.x));
            promises.push(writer.write(`${prefix}.y`, b.y));
            promises.push(writer.write(`${prefix}.w`, b.w));
            // MAP 'd' (depth) in JS to 'l' (length) in PLC struct
            promises.push(writer.write(`${prefix}.l`, b.d));
            promises.push(writer.write(`${prefix}.rot`, b.rot));
        });
